...
const initDB = async () => {
  try {
    ...
    // FORCE RE-SEEDING TO GUARANTEE DATA IS THERE
    console.log('🔄 Force seeding real Kenyan water data...');
    ...
    for (const report of KENYA_WATER_DATA.communityReports) {
      await pool.query(`INSERT INTO community_reports (id, user_id, title, description, county, status, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7)`, [report.id, report.user_id, report.title, report.description, report.county, report.status, report.created_at]);
    }
...
// DATASET ENDPOINTS (THE MISSING PIECE)
