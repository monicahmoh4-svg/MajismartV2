...
// Helper: Check if a column exists in the assets table
...
// GET /api/assets - Resilient, works with or without enhanced migration
router.get('/', async (req, res) => {
  try {
    const { county, type, status, condition, search, limit = 100, offset = 0 } = req.query;
    
    // Dynamically build query based on what columns exist
    const hasCondition = await columnExists('condition');
    const hasInstallationDate = await columnExists('installation_date');
    const hasExpectedLifespan = await columnExists('expected_lifespan_years');
    const hasNextMaintenance = await columnExists('next_maintenance_date');
    
    // Build SELECT clause dynamically
    let selectColumns = `
      a.id, a.name, a.type, a.status, a.latitude, a.longitude, a.county, 
      a.capacity, a.diameter_mm, a.material, a.manufacturer, a.serial_number, 
      a.created_at, a.updated_at
    `;
    
    if (hasCondition) selectColumns += `, a.condition`;
    if (hasInstallationDate) selectColumns += `, a.installation_date`;
    if (hasExpectedLifespan) selectColumns += `, a.expected_lifespan_years`;
    if (hasNextMaintenance) selectColumns += `, a.next_maintenance_date`;
    
    if (hasInstallationDate) {
      selectColumns += `, EXTRACT(YEAR FROM AGE(NOW(), a.installation_date))::INTEGER as age_years`;
    }
    
    // Add subquery counts only if tables exist
    const hasMaintenanceTable = await tableExists('asset_maintenance');
    const hasInspectionsTable = await tableExists('asset_inspections');
    const hasAttachmentsTable = await tableExists('asset_attachments');
    ...
    if (hasInspectionsTable) {
      selectColumns += `, (SELECT COUNT(*) FROM asset_inspections WHERE asset_id = a.id) as inspection_count`;
    } else {
      selectColumns += `, 0 as inspection_count`;
    }
    
    if (hasAttachmentsTable) {
      selectColumns += `, (SELECT COUNT(*) FROM asset_attachments WHERE asset_id = a.id) as attachment_count`;
    } else {
      selectColumns += `, 0 as attachment_count`;
    }
...
// GET /api/assets/stats - Resilient
router.get('/stats', async (req, res) => {
  try {
    ...
    if (hasNextMaintenance) {
      baseStats += `, COUNT(*) FILTER (WHERE next_maintenance_date < NOW()) as overdue_maintenance`;
    } else {
      baseStats += `, 0 as overdue_maintenance`;
    }
    
    if (hasNextInspection) {
      baseStats += `, COUNT(*) FILTER (WHERE next_inspection_date < NOW()) as overdue_inspections`;
    } else {
      baseStats += `, 0 as overdue_inspections`;
    }
    
    if (hasWarranty) {
      baseStats += `, COUNT(*) FILTER (WHERE warranty_expires < NOW() + INTERVAL '30 days' AND warranty_expires IS NOT NULL) as warranty_expiring`;
    } else {
      baseStats += `, 0 as warranty_expiring`;
    }
    
    if (hasMaintenanceTable) {
      baseStats += `, COALESCE((SELECT SUM(cost_ksh) FROM asset_maintenance), 0) as total_maintenance_cost`;
    } else {
      baseStats += `, 0 as total_maintenance_cost`;
    }
    
    if (hasInstallationDate) {
      baseStats += `, AVG(EXTRACT(YEAR FROM AGE(NOW(), installation_date))) as average_age_years`;
    } else {
      baseStats += `, 0 as average_age_years`;
    }
    ...
    const { rows: byCounty } = await db.query(`
      SELECT county, COUNT(*) as count FROM assets WHERE county IS NOT NULL GROUP BY county ORDER BY count DESC
    `);

    let byCondition = [];
    if (hasCondition) {
      const { rows: condRows } = await db.query(`
        SELECT condition, COUNT(*) as count FROM assets GROUP BY condition ORDER BY count DESC
      `);
...
// GET /api/assets/alerts - Resilient
router.get('/alerts', async (req, res) => {
  try {
    ...
    // If no enhanced columns exist, return empty alerts
    if (!hasCondition && !hasNextMaintenance && !hasNextInspection) {
      return res.json({ alerts: [] });
    }
    
    let whereClause = 'WHERE 1=0'; // Default: no alerts
    const conditions = [];
    ...
    if (hasWarranty) conditions.push('(warranty_expires IS NOT NULL AND warranty_expires < NOW() + INTERVAL \'30 days\')');
    ...
    if (hasWarranty) selectClause += ', warranty_expires';
    
    const { rows } = await db.query(`
      ${selectClause} FROM assets ${whereClause}
...
// GET /api/assets/:id - Full asset details (resilient)
router.get('/:id', async (req, res) => {
  try {
    ...
    // Fetch related data only if tables exist
...
// POST /api/assets - Create asset (resilient)
...
// PUT /api/assets/:id - Update asset (resilient)
router.put('/:id', async (req, res) => {
  try {
    ...
    const baseFields = ['name', 'type', 'latitude', 'longitude', 'county', 'status', 
                        'capacity', 'diameter_mm', 'material', 'manufacturer', 'serial_number'];
    const enhancedFields = ['condition', 'installation_date', 'expected_lifespan_years', 
...
// POST /api/assets/:id/maintenance - Log maintenance (only if table exists)
router.post('/:id/maintenance', async (req, res) => {
  try {
    if (!(await tableExists('asset_maintenance'))) {
      return res.status(503).json({ error: 'Maintenance tracking not available. Run migration first.' });
    }
...
// POST /api/assets/:id/inspection - Log inspection (only if table exists)
router.post('/:id/inspection', async (req, res) => {
  try {
    if (!(await tableExists('asset_inspections'))) {
      return res.status(503).json({ error: 'Inspection tracking not available. Run migration first.' });
    }
...
// POST /api/assets/:id/attachments - Upload attachment (only if table exists)
router.post('/:id/attachments', async (req, res) => {
  try {
    if (!(await tableExists('asset_attachments'))) {
      return res.status(503).json({ error: 'Attachment storage not available. Run migration first.' });
    }
