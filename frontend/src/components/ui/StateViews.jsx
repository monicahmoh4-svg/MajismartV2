  ...
  return (
    <div style={{display:'grid',gap:14}}>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))',gap:14}}>
        {Array.from({length:rows}).map((_,i)=><div key={i} className="card skeleton-card" style={{padding:'18px 20px',height:86}}/>)}
      </div>
...
export function ErrorState({ message, onRetry }) {
  return (
    <div className="card fade-in" style={{padding:'32px 20px',textAlign:'center',color:'var(--gray-600)'}}>
      ...
      <div style={{fontSize:13,marginBottom:14}}>{message||'Something went wrong.'}</div>
      {onRetry&&<button className="btn btn-outline" onClick={onRetry} style={{margin:'0 auto'}}><RefreshCw size={14}/> Retry</button>}
    </div>
  )
}
  ...
  return (
    <div className="card fade-in" style={{padding:'18px 20px'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
        ...
        {Icon&&<div style={{width:38,height:38,borderRadius:10,background:bg,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><Icon size={18} color={color}/></div>}
      </div>
    </div>
  )
}
export function LiveBadge({ lastUpdated }) {
  ...
  return (
    <span style={{display:'inline-flex',alignItems:'center',gap:5,fontSize:11,color:'var(--gray-400)'}}>
      <span style={{width:7,height:7,borderRadius:'50%',background:'var(--teal)',display:'inline-block',animation:'pulse-dot 1.6s ease-in-out infinite'}}/>
