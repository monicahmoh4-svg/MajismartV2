...
export default function CitizenDashboard() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [areaData, pointsData, spendingData, reportsData, alertsData, communityData, qualityData] = await Promise.all([
        ...
        api.get('/reports-enhanced').catch(() => []),
      ...
      setMyReports(allReports.filter(r => r && r.reporter_name === user?.name))
  ...
  // ✅ UPDATED: Points to the working /reports-enhanced endpoint with proper payload
  const submitReport = async (e) => {
    e.preventDefault()
    setSubmitting(true)
    try {
      await api.post('/reports-enhanced', { 
  ...
  const getReportStatus = (status) => {
    const statuses = {
      submitted: { label: 'Submitted', color: '#4338ca', bg: '#e0e7ff' },
                      ...
                      return (
                        <div key={i} style={{ padding: '20px', borderBottom: i === myReports.length - 1 ? 'none' : '1px solid #f1f5f9' }}>
                          ...
                          <div style={{ display: 'flex', gap: '16px', fontSize: '13px', color: '#64748b', flexWrap: 'wrap' }}>
                            ...
                            <span>•</span>
                            <span>{new Date(report.submitted_at || report.created_at).toLocaleDateString()}</span>
                          </div>
                        </div>
                      )
                    }) : (
